...
def setup_tc_catalogs(input_data, map_gw, input_rnd, weights=None, use_map=False):
    ...
    cat_gw = tc.Catalog(ra=ra, dec=dec, k=map_gw, ra_units='deg', dec_units='deg', npatch=120)
    if use_map:
        nside = hp.get_nside(map_gw)
        ...
        cat_galaxy = tc.Catalog(ra=input_data['RA'], dec=input_data['DEC'], w=weights, ra_units='deg', dec_units='deg', npatch=120)
...
def compute_corr(cat_galaxy, cat_gw, cat_rnd, min_sep=0.5, max_sep=60, nbins=30, linear=False):
    if linear:
        ...
        binning='Log'
    dd = tc.NKCorrelation(min_sep=min_sep, max_sep=max_sep, nbins=nbins, sep_units='deg', bin_type=binning, var_method='jackknife')
    dr = tc.NNCorrelation(min_sep=min_sep, max_sep=max_sep, nbins=nbins, sep_units='deg', bin_type=binning, var_method='jackknife')
    rd = tc.NKCorrelation(min_sep=min_sep, max_sep=max_sep, nbins=nbins, sep_units='deg', bin_type=binning, var_method='jackknife')
    dd2 = tc.KKCorrelation(min_sep=min_sep, max_sep=max_sep, nbins=nbins, sep_units='deg', bin_type=binning, var_method='jackknife')
    ...
    cov_cross = dd.estimate_cov('jackknife')
    cov_auto = dd2.estimate_cov('jackknife')
    return np.exp(dd.meanlogr), xi, xi_auto, cov_cross, cov_auto
...
for i in range(len(args.zbins)-1):
    ...
    theta, w, w_auto, cov_cross, cov_auto = compute_corr(cat_galaxy, cat_gw, cat_rnd, linear=args.linear, nbins=args.tbins)
    if args.debug:
        plt.figure()
        plt.loglog(theta, w)
        plt.show()
    ...
    np.save(args.out_path.replace('.fits.gz', f'cov_cross_bin{i}'), cov_cross)
    np.save(args.out_path.replace('.fits.gz', f'cov_auto_bin{i}'), cov_auto)
    #if args.n_jk > 0:
